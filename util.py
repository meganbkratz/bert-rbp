...
def load_performance_data(filename):
    """Loads data saved from create_roc_files.py - Returns a stats array where 
    each row has (threshold, true_positives, false_positives, true_negatives, 
    false_negatives, positives, negatives)"""
...
def calculate_precision_recall_fscore(stats):
    """Given a stats array where each row has (threshold, true_positives, 
    false_positives, true_negatives, false_negatives, positives, negatives)
    as generated by create_roc_files.py, return an array of 
...
def get_threshold(stats, min_recall=0.9, min_precision=0.1, mode=None):
    ...
    if mode not in ['lowest', 'high_f0.2']:
        raise Exception("Please specify a mode for finding a threshold. Current options are: 'lowest' (return the lowest valid threshold) and 'high_f0.2' (return the threshold with the highest f0.2 score that meets recall/precision criteria)")
